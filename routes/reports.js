...
router.post('/', [auth], [
  ...
  body('nextWeekTargets').isArray({ min: 1 }).withMessage('At least one target for next week is required')
], async (req, res) => {
  try {
    ...
    const { brand, deliverables, nextWeekTargets, additionalNotes, kpis } = req.body;
    ...
    if (report) {
      ...
      report.nextWeekTargets = nextWeekTargets;
      report.additionalNotes = additionalNotes || '';
      report.kpis = kpis || report.kpis;
...
router.get('/my-reports', auth, async (req, res) => {
  try {
    ...
    const total = await WeeklyReport.countDocuments({ userId: req.user._id });
...
router.get('/current-week', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Server error while fetching current week report' });
  }
});
...
router.get('/:reportId/export', auth, async (req, res) => {
  try {
    ...
    const filename = `Weekly_Report_${report.userId.name.replace(/\s+/g, '_')}_${report.weekStartDate.toISOString().split('T')[0]}.pdf`;
    ...
    doc.text(`Week: ${report.weekStartDate.toDateString()} - ${report.weekEndDate.toDateString()}`, 50, 160);
    ...
    report.deliverables.forEach((deliverable, index) => {
      ...
      doc.text(`   Description: ${deliverable.description}`, 70, yPos + 15);
    ...
    report.nextWeekTargets.forEach((target, index) => {
      ...
      doc.text(`   Due Date: ${new Date(target.dueDate).toDateString()}`, 70, yPos + 30);
    ...
    if (report.kpis && Object.keys(report.kpis).length > 0) {
      ...
      if (report.kpis.engagementRate) doc.text(`Engagement Rate: ${report.kpis.engagementRate}%`, 70, yPos), yPos += 20;
      if (report.kpis.reach) doc.text(`Reach: ${report.kpis.reach}`, 70, yPos), yPos += 20;
      if (report.kpis.conversions) doc.text(`Conversions: ${report.kpis.conversions}`, 70, yPos), yPos += 20;
    }
